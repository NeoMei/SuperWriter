...
SuperWriter 是面向结构化专业长文的协作写作 skill，可用于标书、技术方案、项目建议书、研究报告和白皮书的讨论、组织与撰写；交付验收支持标书及显式登记的非投标专业文档。公开名称保持 **SuperWriter**，内部 skill ID 为 `superwriter`，当前 skill 版本为 `0.2.9`；协作流程协议为 v2。
...
投标场景先提取并锁定招标文件的响应结构，大纲阶段核对规定章节并只在明确允许处细化；评分点映射到规定位置。完整固定、部分固定与自主设计的处理见 [招标响应结构模板](references/招标响应结构模板.md)。机器验收不自动判断招标原文合规，实际输出须另做逐项结构核对。

v0.2.9 交付契约扩展：[持久运行环境](references/运行环境.md)、[结构化核验与附件生命周期](references/结构化核验.md)、[非投标验收](references/专业文档验收.md)。投标 v2 交付必须绑定版式；招标提供 DOCX/DOTX 模板时复制模板并以复制件作为生成基准，验收模板稳定部件和正文实际格式。已发布版本能力以对应标签为准。
...
- 增加招标文件版式规格约束：在结构模板与指导文件中确立版式与结构同等强制原则。
- 结构化核验支持可选 `layout` 字段，覆盖纸张尺寸与方向、页边距、页码及正文字体字号行距。
- 交付验收器核对 DOCX 实际版式与声明规格（sectPr 纸张及页边距 ±0.5mm、原生长文页码、默认段落字体与行距）。
- 保持无 layout 旧大纲与 v1 冻结契约兼容；格式与规格不符时明确以 "tender layout" 报错阻断交付。

### v0.2.9 (2026-09-18)

- protocol v2 投标交付强制绑定已验证的 `layout`，缺少版式声明时直接阻断验收。
- 版式验收检查正文实际段落与字符的字体、字号、行距，避免只改默认样式而绕过检查。
...
- 大纲可绑定规定标题、材料范围/有效期声明及附件摘要；文件或材料变化使相关确认和交付失效。
- 非投标文档使用明确的 professional 类型与需求矩阵验收，保留审批和原生产物检查。
- 保持旧 v2 无扩展项目兼容及 v1 冻结契约；核验声明与文件一致性，不代替材料真实性或招标语义审查。
- 本地 286 项回归通过，2 项 Windows 专用测试跳过；安装、产物验收及独立复审通过。原生产物集成复用既有 WPS 样本，未重新导出。

### v0.2.6 (2026-09-14)

- 新增投标证明材料台账，分别记录真实性与可用性、本项目适用性及逐条支撑程度。
- 串联招标条款、响应陈述、材料摘要、原件页码和最终附件位置；按类别核对主体、产品版本、时间及证明范围。
- 在现有交付报告中核对附件完整性、清晰度、编号与顺序，不新增审批阶段。
- 修正素材打标规范中旧版“先占位、后补材料”的冲突要求。新增规则不代表机器自动校验证书或独立附件。

### v0.2.5 (2026-09-14)

- 标书按招标规定的章节、编号、顺序、层级、表格及附件响应，仅在明确允许的位置扩展。
- 新增响应结构模板及合稿、DOCX/PDF 逐项核对要求；机器 PASS 不等于招标格式合规。
- 同一阶段当前版本的确认同时满足通用设计审批，保留新版本、后续阶段及依赖失效后的审阅要求。
- 安装器同步确认去重路由，避免再次安装覆盖该规则。
