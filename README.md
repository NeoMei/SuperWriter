...
SuperWriter 是面向结构化专业长文的协作写作 skill，可用于标书、技术方案、项目建议书、研究报告和白皮书的讨论、组织与撰写；当前完整交付验收仍以标书场景为基础。公开名称保持 **SuperWriter**，内部 skill ID 为 `superwriter`，当前 skill 版本为 `0.2.6`；协作流程协议为 v2。
...
本地源码新增的交付契约扩展（尚未发布）：[持久运行环境](references/运行环境.md)、[结构化核验与附件生命周期](references/结构化核验.md)、[非投标验收](references/专业文档验收.md)。已发布版本能力以对应标签为准。
