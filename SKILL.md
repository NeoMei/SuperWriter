...
接到任务先确定/创建客户工作区（结构见下），读取 `<标段名>/流水线状态.md`（不存在则初始化为阶段 0）。每完成一阶段更新该文件。阶段 0 是启动预处理；阶段 1–9 是九个业务阶段。门未过不得进入下一阶段；系统会在关键节点主动分析内容并与用户交流，确保写作符合预期。
...
[交互 0·需求收集]：解析招标文件后，系统主动分析并生成"需求摘要"，向用户展示分析结果，询问：
- 写作风格偏好（正式/非正式、技术性/通俗性）
- 重点强调的内容（哪些评分点更重要）
- 特殊要求（行业术语、公司文化、品牌调性等）
- 配图风格偏好（简约/丰富、图表/图片）
- 排版偏好（紧凑/宽松、传统/现代）
生成 `用户偏好配置.json`，用于后续阶段。
...
[交互 2·需求确认]：素材入库后，系统主动分析素材覆盖度，向用户展示分析结果，询问：
- 哪些素材需要重点使用
- 哪些素材需要补充
- 响应策略偏好（实质性/正偏离/负偏离）
深访阶段，系统主动分析评分点，询问：
...
[交互 3·大纲确认]：大纲生成后，系统主动分析大纲结构，向用户展示分析结果，询问：
- 章节结构是否合理
- 内容重点是否正确
- 是否需要调整
...
[交互 4·写作反馈]：每章写作完成后，系统主动分析内容，向用户展示分析结果，询问：
- 内容深度是否合适
- 语言风格是否符合预期
- 是否需要补充说明

**阶段 5 章节核查**：逐章对照矩阵核查覆盖度与响应策略写法。
[交互 5·内容质量确认]：章节核查后，系统主动分析内容质量，向用户展示分析结果，询问：
- 内容是否符合预期
- 是否需要修改
- 是否需要补充
系统主动分析内容一致性，询问：
...
**阶段 6 配图**：默认用 ai-image-to-ppt（Gemini nano banana 2 / doubao-seedream）批量生成配图，产出 JPG 入 `配图/`。仅当客户或标段要求矢量可编辑源时，退化到 obsidian-excalidraw + SVG + PNG 链路。
[交互 6·配图确认]：配图生成后，系统主动分析配图质量，向用户展示分析结果，询问：
- 配图风格是否符合预期
- 是否需要调整
- 是否需要补充
...
[交互 7·合并稿确认]：合并稿生成后，系统主动分析内容一致性，向用户展示分析结果，询问：
- 术语是否一致
- 风格是否统一
- 是否需要调整
...
[交互 8·终稿确认]：终稿审定后，系统主动分析排版质量，向用户展示分析结果，询问：
- 排版是否符合预期
- 是否需要调整
- 是否需要补充
...
[交互 9·导出确认]：导出后，系统主动分析导出质量，向用户展示分析结果，询问：
- 格式是否正确
- 是否需要调整
- 是否需要补充
[交付验收]：生成/更新项目内 `验收清单.json`，其中 `outputs.merged` 只能是项目根 `合并稿.md`；记录当前合并稿 SHA-256、评分点、章节、交付路径及页约束。默认 ai-image-to-ppt 图仅记录 JPG/PNG 渲染路径、标题和摘要；退化的 Excalidraw 图额外记录源文件、SVG 渲染摘要、节点与有向边拓扑。检查 docx 与 pdf 均为当前合并稿导出、双向无新增/遗漏实质正文、可打开且无乱码。这是交付验收，不是流程门。清单字段契约见 `references/验收清单模板.json`。
...
2. 人工确认点：门 2（访谈共识）、门 5（补素材）、门 8（终稿审定）；交互环节主动分析内容并与用户交流
