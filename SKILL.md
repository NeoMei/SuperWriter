...
解析招标文件和评分表，建立需求摘要、应答矩阵、材料登记与 brief。核对评分点总数、硬性要求和交付边界。投标场景按结构模板提取投标文件格式、分册、标题、编号、顺序、层级、必填表格、附件、版式规格及适用补遗，记录来源文件版本和页码/条款；结构约束表与版式规格表作为 brief 正文的一部分，提取不是提前设计大纲。展示已掌握事实、缺口及推荐的访谈重点；记录偏好但不把偏好当批准。完成后写入 brief，并按 `next` 的建议进入方案讨论。
