...
标书先提取招标文件规定的响应结构与版式规格，再在允许范围内细化。招标文件及适用补遗的格式要求优先于写作方案、评分点排序和用户的排版偏好，版式规定与结构规定同等强制；用户确认不能消除格式冲突。不得自行改名、改号、重排、合并或删除规定章节，也不得把招标文件自身目录当作投标文件目录。

投标场景必须读取 `references/招标响应结构模板.md`，区分“完整固定 / 部分固定 / 自主设计”：完整固定按原结构填充；部分固定只在有来源依据的允许位置扩展；未规定结构且已核验相关材料后才自主设计。扩展权限不明时保留原结构，在已有内容位置用段落响应，记录待核验问题，不默认允许新增标题。招标文件提供 DOCX/DOTX 等格式模板时，先将原文件复制到项目内模板路径并登记 SHA-256，后续生成以该复制件为基准套用模板；不得改用自定义 preset。非投标文档按已确认目标组织大纲；有客户指定模板时同样遵守模板。

固定章节必须以招标文件来源为准：在可选的 `outline.metadata.checks.tender_contract` 中登记来源定位、原文标题和固定顺序。评分项顺序只能用于来源明确允许的扩展位置；评分点不能把固定章节挪到扩展位置，也不能因分值高低改名、改号或重排固定章节。合同中的每个评分项保留精确的 `source_label`，已知错别字只能登记为显式 `aliases`，不得静默改写来源文本。
...
投标项目启用 `tender_contract` 后，`review_index.rows` 是逐项的来源顺序索引；草稿可将页码留作占位，但 delivery 必须以 `strict_pages=True` 检查每一项已填入正整数页码。写作阶段可使用 `build_writer_checklist` 生成按来源顺序的 writer checklist，并用 `reverse_evidence_map` 从每个材料 ID 反查所支撑的评分项；这两个结果只是审阅辅助，不构成批准或交付完成。

## 七阶段操作

### intake

解析招标文件和评分表，建立需求摘要、应答矩阵、材料登记与 brief。核对评分点总数、硬性要求和交付边界。投标场景按结构模板提取投标文件格式、分册、标题、编号、顺序、层级、必填表格、附件、版式规格、可复制的 Office 模板及适用补遗，记录来源文件版本和页码/条款；如果没有可复用模板，登记已核验未提供及核验范围。结构约束表与版式规格表作为 brief 正文的一部分，提取不是提前设计大纲。展示已掌握事实、缺口及推荐的访谈重点；记录偏好但不把偏好当批准。完成后写入 brief，并按 `next` 的建议进入方案讨论。
...
本阶段为“响应结构核对与细化”：投标大纲先复制 brief 中的规定结构，只在明确允许的位置细化；无规定结构或非投标文档才依据已批准方案设计完整大纲。投标时将结构约束表与版式规格表完整纳入 outline 正文，记录锁定项、允许扩展依据、原文位置和模板绑定；`outline.metadata.checks.layout` 必须明确 `template.mode=copy` 或 `template.mode=none`，使“有模板已复制”或“已核验无模板”随当前版本及 SHA-256 一起审阅。启用 `tender_contract` 时同时记录固定章节、评分项 `source_label`/`aliases`、review-index 行和证据 ID，并展示 writer checklist 与 reverse evidence map。保持“要求 → 章节 → 依据”追溯，并为每章写明目标、观点、评分点、证据、缺口、篇幅和拟配图。评分点映射到既定响应位置，不为评分或叙事效果更换规定标题；本次承接的分册范围和外部提供的必需部分须明示。用户核对提取完整性、响应安排及允许扩展部分；发现格式冲突先修正，不能用用户批准豁免。展示结构与重点；修改后生成新版本并重新审阅。用户明确确认当前大纲后才能开始正文。大纲修订时，只有用户在同一批准事件中明确列出的、身份与字节未变且没有其他失效原因的章节可保留原确认；其余章节重新审阅。
...
先创建独立 delivery 草稿/报告。投标项目必须有已验证的 `layout`；招标文件有 DOCX/DOTX 模板时，将模板复制到项目内、核对复制件 SHA-256，并以复制件作为 WPSComposer 生成基准，禁止使用会覆盖模板的设计 preset。只有存在需要用户判断的版式对象时，展示实际 WPS 预览、按反馈修改并取得 layout 当前版本确认；没有 layout 对象就阻塞交付。若 WPSComposer 当前公开契约无法接收模板作为生成基准，记录导出能力阻塞并停止，不以模板文件存在或内容可读放行。用模板基准生成 DOCX/PDF，并运行实际文件验收；验收器检查模板稳定 OOXML 部件、页面设置和正文段落/字符的实际格式。投标输出还须按 `references/协作交付验收.md` 逐项核对实际 DOCX/PDF 与招标结构和版式规格，保存来源和输出位置证据；未通过不得记录完成。现有验收器不自动判断招标原文合规，PASS 不能代替这项核对。验收程序通过后，由 agent 通道以当前 delivery 身份、当前合稿摘要及 DOCX/PDF 实际路径和摘要记录 `record_delivery`。交付只有 `verified` 才完成；输出丢失、替换或上游失效后重新生成和验收，旧记录不会自动恢复。
