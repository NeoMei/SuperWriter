...
class DependencyContractTest(unittest.TestCase):
    def test_manifest_validation_uses_the_declared_release_version(self):
        module = load_checker()
        manifest = json.loads((ROOT / "references" / "依赖清单.json").read_text(encoding="utf-8"))
        manifest["superwriter_version"] = "1.2.3"
        with mock.patch.object(module, "SUPERWRITER_VERSION", "1.2.3"):
            self.assertEqual(module.validate_manifest(manifest), [])
            manifest["superwriter_version"] = "1.2.2"
            self.assertIn("superwriter_version must be 1.2.3", module.validate_manifest(manifest))
    ...
    def test_dependency_manifest_matches_release_contract(self):
        ...
        self.assertEqual(manifest["superwriter_version"], load_checker().SUPERWRITER_VERSION)
    ...
    def test_skill_and_readme_publish_the_same_versions_and_dependency_ids(self):
        ...
        self.assertIn(f"version: {load_checker().SUPERWRITER_VERSION}", skill)
        self.assertIn(f"### v{load_checker().SUPERWRITER_VERSION} (", readme)
    ...
    def test_readme_documents_a_portable_version_query(self):
        ...
        self.assertEqual(result.stdout.strip(), f"version: {load_checker().SUPERWRITER_VERSION}")
